...
export default function Woche6() {
  const defaultTasks = [
    { day: 'Montag', items: [] },
    { day: 'Dienstag', items: [] },
    { day: 'Mittwoch', items: [] },
  ...
  const [wochenziel, setWochenziel] = useState(
    'Musikalische Reife und Bühnenvorbereitung'
  );

  // 📥 Daten beim Start laden
  useEffect(() => {
    try {
      const saved = localStorage.getItem('woche6_tasks');
      if (saved) {
      ...
      console.warn('Fehler beim Laden von Woche 6:', e);
    }

    // Falls keine Daten vorhanden sind
    localStorage.setItem('woche6_tasks', JSON.stringify(defaultTasks));
  ...
  useEffect(() => {
    if (isLoaded && tasks) {
      localStorage.setItem('woche6_tasks', JSON.stringify(tasks));
    ...
    <div className="min-h-screen bg-pink-100 p-6">
      <div className="max-w-5xl mx-auto bg-white p-6 rounded-2xl shadow-lg">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <h1 className="text-2xl font-bold text-pink-600">Woche 6</h1>
        </div>
