...
export default function Woche1() {
  ...
  useEffect(() => {
    try {
      ...
      console.warn('Fehler beim Laden von Woche 1:', e);
    }
    // Wenn keine Daten existieren, Default speichern
    localStorage.setItem('woche1_tasks', JSON.stringify(defaultTasks));
  ...
  useEffect(() => {
    if (isLoaded && tasks) {
      localStorage.setItem('woche1_tasks', JSON.stringify(tasks));
  ...
  // ⏳ Ladeanzeige bis Daten sicher geladen sind
